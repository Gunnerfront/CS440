...
class TabQPolicy(QPolicy):
    ...
    def qvals(self, states):
        ...
        return np.array([self.model[disc]])
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        """
        curr_disc = self.discretize(state)
        curr_q = self.model[curr_disc][action]

        target = reward
        if not done:
            next_disc = self.discretize(next_state)
            target += self.gamma * (np.max(self.model[next_disc]))

        new_q = self.model[curr_disc][action] = curr_q + self.lr * (target - curr_q)
        #self.lr = min(self.lr, .1)
        return (new_q - target) ** 2
...
if __name__ == '__main__':
    ...
    # TODO: change buckets
    bckts = (8, 10, 8, 10)
    policy = TabQPolicy(env, buckets = bckts, actionsize=actionsize, lr=args.lr, gamma=args.gamma)
